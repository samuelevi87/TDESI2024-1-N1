...
//    }
        ...
        private static void atividade08(){
            ...
            loja.adicionarProduto(new ProdutoFisico("Laptop", "Ultrafino com 16GB RAM", 2599.99, 30, 1.2, "35x24x1.8 cm"));
            loja.adicionarProduto(new ProdutoFisico("Cadeira Gamer", "Ergonômica com ajuste de altura", 599.99, 70, 15.0, "120x65x60 cm"));
            loja.adicionarProduto(new ProdutoDigital("Software de Edição", "Licença vitalícia", 399.99, 200, 1.0, "EXE"));
            loja.adicionarProduto(new ProdutoDigital("Curso de Design Gráfico", "Complete sua formação", 299.99, 1000, 500, "Video"));
            loja.adicionarProduto(new Servico("Manutenção de Computadores", "Serviço especializado em hardware", 200.00, 15, 3));

            System.out.println("Buscando produtos com 'Software':");
            loja.buscarPorNome("Software").forEach(System.out::println);

            System.out.println("\nListando produtos físicos:");
            loja.listarPorCategoria(ProdutoFisico.class).forEach(System.out::println);

            Produto laptop = loja.buscarPorNome("Laptop").get(0);
            Produto softwareEdicao = loja.buscarPorNome("Software de Edição").get(0);
            carrinho.adicionarProduto(laptop);
            carrinho.adicionarProduto(softwareEdicao);
            ...
            System.out.println("Buscando produtos com 'Curso':");
            loja.buscarPorNome("Curso").forEach(System.out::println);

            System.out.println("\nListando serviços:");
            loja.listarPorCategoria(Servico.class).forEach(System.out::println);
